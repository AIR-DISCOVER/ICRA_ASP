...
def find_nearest_node(position_graph, predicted_next_position,mask_selected_padding):
    ...
    EUdist2 = torch.pow(diff, 2).sum(-1).squeeze(0)*mask_selected_padding
    print(EUdist2)
    return EUdist2.argmin()
...
path = 'saved_model/p2p_blockM_position_type.pickle'
...
for linkM, blockM, position_graph, position_seq, position_real, node_type, order in loader:
    ...
    for i in range(2,realnum_of_node+1):   
        for kk in range(k):
            position = model(position_graph, node_type, blockM, position_seq[kk,:,:].unsqueeze(0))
    ...
    txt_file = open('test/p2p_blockM_position_type/lsd.txt', "a", encoding="utf-8")
    txt_file.write(str(lsd))
    txt_file.write("\n") 
    txt_file = open('test/p2p_blockM_position_type/tua.txt', "a", encoding="utf-8")
