...
def read_data(filename):   
    ...
    return linkM, position, output_pos


def shuffle_read_train(filename): 
    f=open(filename, 'r')
    a=f.read().split(',')
    sh_list=[]
    for i in a:
...
def make_data(n_iter): 
    ...
    for i in range(n_iter):
        ...
        linkM_item, position_item, output_pos_item = read_data(filename)
    ...
    return linkM, position_enc_input, position_dec_input, position_dec_output


class Mydataset(Data.Dataset):
    def __init__(self, linkM, position_graph, position_seq, position_real):
    ...
    def __getitem__(self, idx):
        return self.linkM[idx].to(device), self.position_graph[idx].to(device), self.position_seq[idx].to(device), \
               self.position_real[idx].to(device)
...
BP = 0
# path='save_epoch/p2p_linkM_position/epoch'+str(BP)+'.pickle'
# model.load_state_dict(torch.load(path))
LR=1e-3
optimizer = optim.SGD(model.parameters(), lr=LR, momentum=0.85) 
linkM, position_graph, position_seq, position_real = make_data(n_iter)
loader = Data.DataLoader(Mydataset(linkM, position_graph, position_seq, position_real), batch_size, True)
start_time = time.time()
loss_save = []

for epoch in range(epochs):
    for linkM, position_graph, position_seq, position_real in loader:
        ...
        txt_file = open('loss/loss_p2p_linkM_position.txt', "a", encoding="utf-8")
    ...
    if epoch % 100 == 1:
        for p in optimizer.param_groups:
            p['lr'] *= 0.95
        path = 'save_epoch/p2p_linkM_position/epoch' + str(BP + epoch - 1) + '.pickle' 
