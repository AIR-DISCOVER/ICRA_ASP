...
for linkM, blockM, position_graph, position_seq, position_real, node_type, order in loader:
    ...
    for i in range(2,realnum_of_node+1):   
        for kk in range(k):
            position = model(position_graph, blockM, position_seq[kk,:,:].unsqueeze(0))
    ...
    txt_file = open('test/p2p_blockM_position/lsd.txt', "a", encoding="utf-8")
    txt_file.write(str(lsd))
    txt_file.write("\n") 
    txt_file = open('test/p2p_blockM_position/tua.txt', "a", encoding="utf-8")
