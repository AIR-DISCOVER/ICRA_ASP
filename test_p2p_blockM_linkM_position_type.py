...
# node feature: position + type
# edge type: blockM + linkM
...
for linkM, blockM, position_graph, position_seq, position_real, node_type, order in loader:
    ...
    masked_ID=torch.tensor([[top_k_indices[0,0]],[top_k_indices[0,1]],[top_k_indices[0,2]], [top_k_indices[0,3]],[top_k_indices[0,4]],[top_k_indices[0,5]]] ) #k=6
    for i in range(2,realnum_of_node+1):   
        for kk in range(k):
            position = model(position_graph, node_type, blockM, linkM, position_seq[kk,:,:].unsqueeze(0))
    ...
    txt_file = open('test/p2p_blockM_linkM_position_type/lsd.txt', "a", encoding="utf-8")
    txt_file.write(str(lsd))
    txt_file.write("\n") 
    txt_file = open('test/p2p_blockM_linkM_position_type/tua.txt', "a", encoding="utf-8")
