...
# @Time : 2022/9/4 
# @Author : Lin
# node feature: position and type
# edge feature: block and link
...
def read_data(filename):  
    ...
    order=data['order']
    blockM=data['blockM']    # n_of_node*n_of_node*6_direction --> 6_direction*n_of_node*n_of_node  
    blockM=torch.transpose(blockM, 0,2)
    node_type=node_type[order.long()]
    output_pos=position[order.long()]   # ground truth position sequence
    return linkM, blockM,position,node_type, output_pos, order

def shuffle_read_train(filename): #filename='shuffle.txt'  shuffle the training data items
    f=open(filename, 'r')
    a=f.read().split(',')
    sh_list=[]
    for i in a:
...
def make_data(n_iter):  
    ...
    for i in range(n_iter):
        ...
        position_dec_input = torch.cat([position_dec_input, torch.tensor(position_added).float().unsqueeze(0)],
                                       dim=0)  
        # position_dec_output + token + padding
        position_added = torch.cat([output_pos_item, torch.ones(padding_len, d_feature) * -1], dim=0)  # -1 is end token 
...
class EncoderLayer(nn.Module):
    def __init__(self):
        ...
        self.enc_sef_atten_0 = MultiHeadAttention()  # different edge features to different attention
...
class Encoder(nn.Module):
    ...
    def forward(self, position,node_type,blockM,linkM):
        ...
        for layer in self.layers:
            enc_embeddings = layer(enc_embeddings, enc_graph_attn_mask, enc_link_attn_mask)
...
class myGraphTransformer(nn.Module):
    ...
    def forward(self, position_graph, node_type, blockM, linkM, position_seq):  
        if hasattr(torch.cuda, 'empty_cache'):
            torch.cuda.empty_cache()
        enc_embedding = self.encoder(position_graph, node_type, blockM,linkM)   
...
def criterion(position_output, position_real):  # squared Euchlidan distance 
    loss = 0
...
BP = 0
# if interrupted reload the saved epoach
...
for epoch in range(epochs):
    for linkM, blockM, position_graph, position_seq, position_real, node_type, order in loader:
        position = model(position_graph, node_type, blockM,linkM, position_seq)
        loss = criterion(position, position_real)
        print('Epoch:', '%04d' % (BP + epoch + 1), 'loss =', '{:.6f}'.format(loss))
        txt_file = open('loss/loss_p2p_blockM_linkM_position_type.txt', "a", encoding="utf-8")
    ...
    if epoch % 100 == 1:
        for p in optimizer.param_groups:
            p['lr'] *= 0.95          
        path = 'save_epoch/p2p_blockM_linkM_position_type/epoch' + str(BP + epoch - 1) + '.pickle'
        torch.save(model.state_dict(), path)    # In case of interruption

end_time = time.time()
time_used = end_time - start_time
print('time cost', time_used, 's')
path = 'saved_model/p2p_blockM_linkM_position_type.pickle'
